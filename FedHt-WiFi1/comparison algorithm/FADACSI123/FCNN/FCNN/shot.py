...
def test_target(args,epoch, netF,netB, netC,test_loader): #目标域测试 这个可以直接用
    netF.eval()
    netB.eval()
    netC.eval()# cl_D.eval()
    correct = 0
...
def train_source(args):
    ...
    for epoch in range(epoches):
        ...
        for i, d in enumerate(train_loader):
            ...
            classifier_loss=loss.CrossEntropyLabelSmooth(num_classes=args.class_num,epsilon=args.smooth)(outputs_source, source_label.long())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--seed', type=int, default=4, help="random seed")#4 haode
