...
class DANN(nn.Module):
    def __init__(self):
        ...
        self.feature = nn.Sequential(nn.Conv2d(in_channels=1, out_channels=32, kernel_size=5, padding=1),
                                  nn.BatchNorm2d(32),
                                  nn.ReLU(True),
        ...
        self.class_d = nn.Sequential(nn.Linear(128*124*1, 1024),
                                   nn.ReLU(True),
                                   nn.Linear(1024, 256),
                                   nn.ReLU(True),
                                   nn.Linear(256, 7))
        self.domain_d = nn.Sequential(nn.Linear(128*124*1, 1024),
                                   nn.ReLU(True),
                                   nn.Linear(1024, 256),
                                   nn.ReLU(True),
                                   nn.Linear(256, 2))
...
def test_target(epoch, net, test_loader):
    net.eval()
    # cl_D.eval()
    correct = 0
    total = 0
    ...
    # if acc >= 0.60:
    #     save_model(net, 'G_model/GAN_2_model/net_'+str(acc)+'.pkl' )
        # save_model(cl_D, 'G_model/GAN_2_model/cl_D_BA_' + str(acc) + '.pkl')
    #with open('acc/DANN_target.txt', 'a+') as f:
        #f.write(str(acc) + str(-epoch) + '\n')
    #f.close()

def test_source(epoch, net, test_loader):
    net.eval()
    # cl_D.eval()
    correct = 0
    total = 0
    ...
    #with open('acc/DANN_source.txt', 'a+') as f:
       # f.write(str(acc) + str(-epoch) + '\n')
    #f.close()
...
def train():
    ...
    for epoch in range(epoches):
        ...
        for i, d in enumerate(train_loader):
            ...
            target_doman_label = Variable(torch.Tensor(d['A'].size(0), 1).fill_(0), requires_grad=False).cuda()
        ...
        test_source(epoch, net, test_loader)
        test_target(epoch, net, test_loader)
