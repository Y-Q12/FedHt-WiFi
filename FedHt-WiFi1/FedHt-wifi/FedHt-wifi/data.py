...
class ImageDataset(Dataset):
    ...
    def __init__(self, root, transforms_=None, unaligned=False, mode='train'):
        ...
        self.files_D = sorted(glob.glob(os.path.join(root, '%s_D' % mode) + '/*.*'))
        self.files_E = sorted(glob.glob(os.path.join(root, '%s_E' % mode) + '/*.*'))
        self.files_F = sorted(glob.glob(os.path.join(root, '%s_F' % mode) + '/*.*'))
    ...
    def __getitem__(self, index):
        ...
        D = np.load(self.files_D[(index) % len(self.files_D)])
        E = np.load(self.files_E[(index) % len(self.files_E)])
        F = np.load(self.files_F[(index) % len(self.files_F)])
        G = np.load(self.files_G[(index) % len(self.files_G)])
       ...
       # label_A = A[0, 1:2]
        ...
        data_D = np.expand_dims(dat_D, 0)
        data_E = np.expand_dims(dat_E, 0)
        data_F = np.expand_dims(dat_F, 0)
        data_G = np.expand_dims(dat_G, 0)

        return {'A': data_A, 'label_A': label_A, 'domain_A': domain_A, 'B': data_B, 'label_B': label_B, 'domain_B':domain_B,
                'C': data_C, 'label_C': label_C, 'domain_C': domain_C,'D': data_D, 'label_D': label_D, 'domain_D': domain_D,'E': data_E, 'label_E': label_E, 'domain_E': domain_E,
               'F': data_F, 'label_F': label_F, 'domain_F': domain_F,'G':data_G, 'label_G':label_G}
...
def Get_dataloader():
    ...
    test_loader_1 = DataLoader(ImageDataset('/home/yq/yqdata2', unaligned=True, mode='test'),
